...
本文件是面向公开展示和部署的说明文档。历史原版说明主体已保存在 [READ.md](READ.md)，当前版本差异补充在该文件文末，仅供功能变更对照。
...
当前版本以 LevelDB 作为高频业务数据的主要存储路径，JSON 仅用于 LevelDB 不可用时的小规模故障降级。不要手工删除 `.ldb`、`CURRENT` 或 `MANIFEST` 文件来缩短启动时间。

| 范围 | 当前行为 | 上限或周期 |
| --- | --- | --- |
| 高级群欢迎消息索引 | 按 Bot 保留；启动先加载群、待处理投诉和投诉黑名单，索引过期清理转为 ready 后后台任务 | 普通索引 24 小时 TTL；每 Bot 50,000 条；超限保留约 90% |
| 机器人自身消息索引 | 按 Bot 保留，用于撤回、引用和近期定位 | 24 小时 TTL；每 Bot 20,000 条；超限保留约 90% |
| 高级群欢迎索引缓存 | 仅缓存近期访问的索引项 | 2,000 条 |
| 发言记录缓存 | LevelDB 记录和统计缓存均为有界 | 记录 20,000 条；统计 5,000 条 |
| 群活跃已知用户缓存 | 仅保留近期去重键 | 20,000 条 |
| 全量消息记录缓存 | 记录缓存和成员昵称均为有界 | 各 20,000 条 |
| 入站消息队列 | 每 Bot 8 个并发处理位；被动全量消息优先被淘汰，管理命令优先保留 | 普通队列 500；高优先级硬上限 1,000；单来源被动消息 100 |

- 高级群欢迎索引清理约在 Store ready 后 30 秒启动，后续按周期检查；写入后可提前请求下一次清理。
- 清理删除前会重新确认 `self_id`、索引时间和 `expire_at`，避免后台扫描删除清理期间已经更新的索引。
- `advancedWelcomeStore.close()` 会等待当前清理和写入队列，并关闭 Level 包装层的定时任务。
- 以上限制保护进程内存和数据库，不改变 DAU、聊天统计、撤回、引用、投诉或欢迎配置的对外接口。

### 用户管理与数据作用域

- 用户、群、群成员、拉黑、注销、发言、群活跃和高级群欢迎配置都按当前 Bot `self_id` 隔离；多个 Bot 不共享名单和统计。
- 频道事件不写入用户管理的群/好友缓存，也不参与用户管理拉黑、查询或群成员关系统计。
- 注销是业务不可用状态，不会物理删除历史数据。管理员应按实际保留需求使用现有查询、清空或管理入口。
- 用户/群拉黑只阻断外部插件分发；内部 `#QQBot` 管理命令始终保留，便于恢复配置。
- 发言、成员关系和 openid 属于本地业务数据。请限制服务器和备份访问权限，并向受影响群成员说明已启用的管理或记录能力。

### 消息防护

消息防护位于高级设置，不影响全局进程队列和内存保护。
...
#QQBot高级设置 消息防护设置 图片模式 开启/关闭
#QQBot高级设置 消息防护观察 开启/关闭
#QQBot高级设置 消息防护观察 查看 [图片|文本]
#QQBot高级设置 消息防护白名单 查看
#QQBot高级设置 消息防护白名单 添加 user_openid
#QQBot高级设置 消息防护白名单 删除 user_openid
#QQBot高级设置 消息防护白名单 清空 确认
#QQBot高级设置 消息防护可信群 查看
#QQBot高级设置 消息防护可信群 添加 group_openid
#QQBot高级设置 消息防护可信群 删除 group_openid
#QQBot高级设置 消息防护可信群 清空 确认
```

- 设置页共 3 页；旧 `消息防护设置 4` 兼容为第 1 页。旧 `消息防护观察 查看 1/2/3/4` 都返回同一份单页总览。
- 默认关闭总开关、观察模式和图片模式。用户级、重复内容、群级、消息 ID 去重和聚合统计默认开启。
- 用户级限制为 1 秒内私聊/普通全量各 5 条、全量 `is_you=true` 7 条、主动 @ 10 条、回调按钮 7 条；相同内容前 3 条允许；单群总量为 1 秒 20 条。
- 白名单和管理员旁路仅在“管理员/白名单旁路”开启时生效；它们不会绕过进程队列保护。可信群仅在“可信群旁路”开启时绕过群级总量，不绕过用户级、重复内容、首次屏蔽或队列保护。
- 白名单和可信群每 Bot 最多 1,000 条，添加会去重和规范化，清空需要 `清空 确认`。
- 观察模式使用与正式防护独立的限流、去重和临时屏蔽状态。它只记录预计拦截，消息仍正常进入处理路径；切换到正式防护不会继承观察态的限制状态。
- 总览区分实际放行、预计拦截、实际拦截、首次屏蔽和队列淘汰。没有统计或关闭聚合统计时显示“不可用”，不会伪装为 0。
- 图片模式只发送脱敏 Bot 标识和聚合数据，不包含消息正文、完整 openid、附件或原始事件。渲染按 Bot 与标题合并并发请求，30 秒超时；渲染或图片发送失败时自动回退文本。

### 已移除或不再支持的旧功能

以下能力在旧版文档或旧实现中出现过，但当前版本不提供。请不要从旧仓库复制文件、配置或命令重新启用它们。

| 旧功能或入口 | 当前状态 | 原因与替代原则 |
| --- | --- | --- |
| `YePanel` | 已移除 | 当前插件不再包含该面板及其运行时代码；不要依赖其 DAU 或设置入口。 |
| `#QQBot一键群发` / `oneKeySendGroupMsg` | 已移除 | 不提供面向全部群的主动广播能力，避免误发、会话限制和频率风险。 |
| `deleteNotExistGroup` 自动删除群记录 | 已移除 | 不自动删除群配置、欢迎记录或用户历史，避免把暂时不可查询的群误判为无效数据。 |
| 旧版 READ 中的自用面板、广播或特定环境示例 | 仅历史参考 | 这些内容不构成当前功能承诺，也不是受支持的部署接口。 |

升级时保留现有 LevelDB、配置和用户数据；不要通过删除旧数据、强制 JSON fallback 或复制旧版运行时代码解决兼容问题。功能差异应以当前命令菜单和本 README 为准。
